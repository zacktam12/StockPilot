...
import { Plus, Download, Upload, Search, FileCog, Truck, FileText, AlertCircle } from "lucide-react";
import Button from "../../../components/shared/Button";
import Input from "../../../components/shared/Input";
import { useOutsideClick } from "../../../hooks/useOutsideClick";
import { parseCSV, validateCSVData, convertCSVToSuppliers, REQUIRED_SUPPLIER_CSV_FIELDS } from "../../../utils/csvUtils";
import { showToast } from "../../../store/slices/uiSlice";
import { openCreateDrawer } from "../../../store/slices/supplierSlice";
import SuppliersFilters from "./SuppliersFilters";

const SuppliersHeader = ({ onExportCSV, searchTerm, onSearchChange, onAddNew }) => {
  const dispatch = useDispatch();
  const [isActionsOpen, setIsActionsOpen] = useState(false);
  const [isImporting, setIsImporting] = useState(false);
  const actionsRef = useRef(null);
  const fileInputRef = useRef(null);
  const { items, selectedItems } = useSelector((state) => state.supplier);
  ...
  const handleExport = () => {
    if (selectedItems && selectedItems.length > 0) {
      ...
      const suppliersToExport = items.filter((supplier) => 
        selectedItems.includes(supplier.id)
      );
      onExportCSV(suppliersToExport);
    } else {
      // Export all items
      onExportCSV(items);
    }
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      // Validate the data
      const errors = validateCSVData(parsed.data, REQUIRED_SUPPLIER_CSV_FIELDS);
      if (errors.length > 0) {
        dispatch(showToast({ 
      ...
      // Convert and import suppliers
      const suppliers = convertCSVToSuppliers(parsed.data);
      const result = await dispatch(importSuppliers(suppliers)).unwrap();
      
      dispatch(showToast({ 
        message: `Successfully imported ${result.importedCount || suppliers.length} suppliers!`, 
        type: "success" 
      }));
              ...
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-80 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-lg border border-gray-200 dark:border-gray-700 shadow-xl z-[9999]">
                  ...
                  <div className="px-6 py-3 border-b border-gray-200">
                    ...
                    <p className="text-sm text-gray-600">Supplier Actions</p>
                  </div>
                  ...
                  <div className="p-3 space-y-2 flex flex-col items-center">
                    ...
                    >
                      <Upload size={18} />
                      <span>
                        Export Suppliers ({selectedItems && selectedItems.length > 0 ? selectedItems.length : items?.length || 0})
                      </span>
                    </button>
                  </div>
                </div>
