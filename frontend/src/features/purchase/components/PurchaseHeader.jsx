...
import { Plus, Download, Upload, Search, FileCog, ShoppingCart, AlertCircle } from "lucide-react";
...
const PurchaseHeader = ({ 
  ...
  const { selectedItems, items: purchases = [] } = useSelector((state) => state.purchases || {});
  ...
  const handleExport = () => {
    // Use purchases from Redux if filteredPurchases prop is empty
    const purchaseData = (filteredPurchases && filteredPurchases.length > 0) ? filteredPurchases : purchases;

    if (selectedItems && selectedItems.length > 0) {
      // Export only selected items
      const purchasesToExport = purchaseData.filter((purchase) => 
        selectedItems.includes(purchase.id)
      );
      exportPurchasesToCSV(purchasesToExport);
    } else {
      // Export all purchases
      exportPurchasesToCSV(purchaseData);
    }
    setIsActionsOpen(false);
  };
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      dispatch(showToast({ 
        message: "Purchase import functionality is not fully implemented yet", 
        type: "info" 
      }));

    } catch (err) {
      dispatch(showToast({ 
        message: err.message || "Failed to import purchases", 
        type: "error" 
      }));
    } finally {
              ...
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-80 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-lg border border-gray-200 dark:border-gray-700 shadow-xl z-[9999]">
                  ...
                  <div className="px-6 py-3 border-b border-gray-200">
                    ...
                    <p className="text-sm text-gray-600">Purchase Actions</p>
                  </div>
                  ...
                  <div className="p-3 space-y-2 flex flex-col items-center">
                    ...
                    >
                      <Download size={18} />
                      <span>
                        Export Purchases ({selectedItems && selectedItems.length > 0 ? selectedItems.length : purchases?.length || 0})
                      </span>
                    </button>
                  </div>
                </div>
